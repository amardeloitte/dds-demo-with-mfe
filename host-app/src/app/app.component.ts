...
import {
  COLUMN_DATA,
  COLUMN_DATA_PAGINATION,
  TABLE_DATA,
  TABLE_DATA_WITH_PAGINATION,
} from './table-data';
...
export class AppComponent {
  ...
  @ViewChild('tableElementPagination')
  tableElementPagination!: ElementRef<HTMLElement>;
  @ViewChild('paginationElement') paginationElement!: ElementRef<HTMLElement>;
  ddsdemomodalInstance: any;
  selectedComponent: string = '';
  isRowSelected: boolean = false;

  constructor(private dataBus: DataMBusService, private http: HttpClient) {}
  ...
  changeComponent(component: any) {
    ...
    switch (component) {
      case 'Modal':
        setTimeout(() => {
          this.ddsdemomodalInstance = new DDS.Modal(
            this.ddsdemomodal.nativeElement
          );
        });
      ...
      case 'Tables':
        ...
        this.dataBus?.setDataFromHost({
          column: column
        });
        this.dataBus?.getDataFromRemote().subscribe((res: any) => {
          this.isRowSelected = res?.idArr?.length > 0;
        })
        break;
      default:
        break;
